...
public class Java_notepad extends JFrame {
    ...
    JButton btnBg;
    JButton bold, italic, bold_it, plain;
    int fsize = 12;
    int fface = 0;
...
//end
    public Java_notepad() {
        ...
        itmExit.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    if (/*mainarea.getText().equals(null)*//*mainarea.getText().equals(filecontent)||*/!(mainarea.getText().equals(null))) {
                        System.exit(0);
                    } else {
                        saveas();
                        System.exit(0);
    ...
    public void open() throws IOException {
        try {
            ...
            if (retval == jc.APPROVE_OPTION) {
                ...
                int nch;
                filename = jc.getSelectedFile().getName();
                if (!filename.endsWith(".cpp") && !filename.endsWith(".txt") && !filename.endsWith(".xml") && !filename.endsWith(".docx") && !filename.endsWith(".java") && !filename.endsWith(".rtf")) {
